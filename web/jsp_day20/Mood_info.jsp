...
<%
  ...
  <h1>心情详情 评论页面</h1>
		...
		<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=1">首页</a> &nbsp;
		<c:if test="${currentPage==1}" var="pre">
			<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=1">上一页</a> &nbsp;
		</c:if>
		<c:if test="${ ! pre }">
			<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=${currentPage-1 }">上一页</a> &nbsp;
		</c:if>
		<c:if test="${currentPage==totalCount}" var="next">
			<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=${totalCount }">下一页</a>&nbsp;
		</c:if>
		<c:if test="${ ! next }">
			<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=${currentPage+1 }">下一页</a>&nbsp;
		</c:if>
		<a href="/jsp_day20/Mood_info.jsp?moodId=${moodId}&currentPage=${totalCount}">末 页</a><br />
