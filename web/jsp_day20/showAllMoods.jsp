...
<%@ page language="java" import="day20.service.UserService" pageEncoding="UTF-8"%>
...
<%
	...
	User user = (User)request.getSession().getAttribute("user");//查到相应的结果
	...
	<script type="text/javascript">
		function doDelete(id){
			if(confirm("是否删除该条数据？")){
				location.href="<%=basePath%>/jsp_day20/Mood_doDelete.jsp?moodId="+id;
			}
		}
	</script>
  </head>
  ...
  <body>
   ...
   <%--  <table border="1">
	...
	<a href="/jsp_day20/Mood_regist.jsp?userName=<%=user.getUserLogin() %>">添加心情记录</a>
	...
	<table border="1"  class="">
		...
		<c:forEach var="mood" items="${some}">
			<tr><td>${mood.moodId }</td><td><a href="/jsp_day20/Mood_info.jsp?moodId=${mood.moodId }">${mood.moodTitle }</a></td>
				<td>${mood.moodContent }</td>
				<td>${mood.moodDate }</td><td>${mood.moodUserlogin }</td>
				<td><a href="/jsp_day20/Comment_regist.jsp?userName=<%=user.getUserLogin() %>&moodId=${mood.moodId }">添加评论</a></td>
				<td><a href="/jsp_day20/Mood_modify.jsp?moodId=${mood.moodId}">修改</a></td>
	...
	<a href="/jsp_day20/showAllMoods.jsp?currentPage=1">首页</a> &nbsp;
	<c:if test="${currentPage==1}" var="pre">
		<a href="/jsp_day20/showAllMoods.jsp?currentPage=1">上一页</a> &nbsp;
	</c:if>
	<c:if test="${ ! pre }">
		<a href="/jsp_day20/showAllMoods.jsp?currentPage=${currentPage-1 }">上一页</a> &nbsp;
	</c:if>
	<c:if test="${currentPage==totalCount}" var="next">
		<a href="/jsp_day20/showAllMoods.jsp?currentPage=${totalCount }">下一页</a>&nbsp;
	</c:if>
	<c:if test="${ ! next }">
		<a href="/jsp_day20/showAllMoods.jsp?currentPage=${currentPage+1 }">下一页</a>&nbsp;
	</c:if>
	<a href="/jsp_day20/showAllMoods.jsp?currentPage=${totalCount}">末 页</a><br />
